...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList =
    descendingByRetweetAcc(this.remove(mostRetweeted), new Cons(mostRetweeted, Nil))

  def descendingByRetweetAcc(ts: TweetSet, tl: TweetList): TweetList =
    if (ts.isEmpty) tl
    else
      descendingByRetweetAcc(ts.remove(ts.mostRetweeted), tl.append(ts.mostRetweeted))
...
class NonEmpty(val elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(t: Tweet, ts: TweetSet): Tweet = {
    val funnel = ts.filter(tweet => tweet.retweets > t.retweets)
    if (funnel.isEmpty ) t
